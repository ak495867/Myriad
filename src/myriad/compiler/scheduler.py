...
def memory_plan(graph: Graph, bytes_per_element: int = 4) -> dict[str, tuple[int, int]]:
    ...
    for name, value in graph.values.items():
        if value.producer is None:
            ...
            start = next(index for index, layer in enumerate(graph.layers) if layer.name == value.producer)
    ...
    for name, start, end, size in intervals:
        ...
        offset = 0
        for active_name, _, _, _ in sorted(active, key=lambda item: placements[item[0]][0]):
            occupied_start, occupied_size = placements[active_name]
            if offset + size <= occupied_start or offset >= occupied_start + occupied_size:
                continue
