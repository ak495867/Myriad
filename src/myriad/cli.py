...
def demo_tensors(seed: int = 7) -> list[Tensor]:
    ...
    return [
        Tensor(
            name="encoder.weight",
            data=rng.normal(0, 0.4, size=(256, 128)).astype(np.float32),
        ),
        Tensor(
            name="projection.weight",
            data=rng.normal(0, 0.2, size=(128, 64)).astype(np.float32),
        ),
        Tensor(
            name="head.weight",
            data=rng.normal(0, 0.1, size=(64, 10)).astype(np.float32),
        ),
    ]
...
def command_compress(args: argparse.Namespace) -> int:
    config = CompressionConfig(
        ...
        block_shape=(
            ...
            if args.block_rows and args.block_cols
            else None
        ),
...
def command_compile(args: argparse.Namespace) -> int:
    ...
    artifact = compile_graph(
        graph, bytes_per_element=args.bytes_per_element, parallelism=args.parallelism
    )
...
def command_roofline(args: argparse.Namespace) -> int:
    ...
    points = roofline_points(
        demo_layers(),
        hardware,
        parameter_bytes=args.bytes_per_element,
        activation_bytes=args.bytes_per_element,
    )
...
def command_cost(args: argparse.Namespace) -> int:
    ...
    payload = silicon_cost(
        estimate,
        wafer_diameter_mm=args.wafer_diameter,
        die_yield=args.yield_rate,
        wafer_cost_usd=args.wafer_cost,
    )
    write_json(payload, args.output)
    print(json.dumps(payload, indent=2))
    return 0
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="myriad",
        description="Silicon model compression and hardware co-design lab",
    )
    ...
    compress.add_argument(
        "--dtype",
        choices=[dtype.value for dtype in DataType if dtype is not DataType.FP32],
        default="int8",
    )
    ...
    sweep_parser.add_argument(
        "--hardware", choices=["edge-npu", "datacenter-npu"], default="edge-npu"
    )
