...
def classify_layer(
    ...
    machine_balance = (
        hardware.peak_macs_per_second / hardware.dram_bandwidth_bytes_per_second
    )
    return (
        "compute_bound"
        if estimate.operational_intensity >= machine_balance
        else "memory_bound"
    )
...
def roofline_points(
    ...
    for layer in layers:
        ...
        attainable = min(
            hardware.peak_macs_per_second,
            estimate.operational_intensity * hardware.dram_bandwidth_bytes_per_second,
        )
        points.append(
            {
                "layer": layer.name,
